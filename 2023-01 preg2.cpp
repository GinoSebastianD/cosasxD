...
struct Node {
    ...
    Node(int* arr, int start, int end) {
        ...
        children[0] = new Node(arr, start , (start + end) / 2); //[0-2] -> [0-1] -> [0-0] ->[1-1]
        children[1] = new Node(arr, (start + end) / 2 + 1 , end); //[3-4] -> [3] -> [4] ->[3-3] ->[4-4]

        sum = children[0]->sum + children[1]->sum; //sumar ambos nodos
