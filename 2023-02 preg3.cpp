...
private:
    ...
    void sobrinos(CBinNode* x, bool eshijo);
    CBinNode* m_root;
    bool m_b;
};
...
void CBinTree::sobrinos(CBinNode* x, bool eshijo)
{
    if (!x) return;
    if (!eshijo)
    {
    ...
    sobrinos(x->nodes[0], false);
    sobrinos(x->nodes[1], false);
}
...
void CBinTree::sobrinos_de(int x)
{
    ...
    sobrinos((*pos)->nodes[0], true);
    sobrinos((*pos)->nodes[1], true);
