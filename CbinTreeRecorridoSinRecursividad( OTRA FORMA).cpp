#include "iostream"
#include "stack"
#include "utility" //para usar std::pair -> otra manera de hacerlo
using namespace std;
...
public:
	...
	void in_orden_sin_recursividad_otraforma(Node* r);
...
void RecorridoArbol::in_orden_sin_recursividad_otraforma(Node* r)
{
	std::stack<std::pair<Node*, int>> st;
	st.push({r,0});
	while ( !st.empty()  )
	{		
		std::pair<Node*, int>& tope = st.top();// creo mi variable de tipo tope y le asigno el elemento q esta en la cima de la pila.
		switch (tope.second)
		{
		case 0:
			tope.second = 1;
			if (tope.first->izq) //diferente de null
			{
				st.push({ tope.first->izq,0 });					//empilamos	
			}
			break;
		case 1:
			cout << tope.first->v<< " ";
		...
		case 2:
			tope.second = 3;
			if (tope.first->der) //diferente de null
			{
				st.push({ tope.first->der,0 });					//empilamos	
			}
			break;
		case 3:
			st.pop();
...
int main() {
	...
	cout << endl;
	arb.in_orden_sin_recursividad_otraforma(arb.root);
	cout << endl;
